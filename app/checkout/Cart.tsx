    ...
    <>
      ...
      <Items>
        {cartItems.map((item, index) => (
          <Item key={`${item.id}-${(item.color as Color).code}-${item.size}`}>
            <ItemImage src={item.image} />
            <ItemDetails>
              <ItemName>{item.name}</ItemName>
              <ItemID>{item.id}</ItemID>
              <ItemColorName>顏色｜{(item.color as Color).name}</ItemColorName>
            ...
            <ItemPrice>
              ...
              <ItemPriceValue>NT.{(item.qty as number) * (item.price as number)}</ItemPriceValue>
            </ItemPrice>
